...
import { completeLesson, getCourseProgress } from "@/lib/services/courseService";
...
const CourseDetail = () => {
  ...
  // Load course progress from Firestore
  useEffect(() => {
            ...
            {activeLesson && course.enrolled && (
              <div className="mb-8">
                <VideoPlayer
                  ...
                  onComplete={async () => {
                    if (!userId || !id || !activeLesson) return;
                    
                    const lessonId = `${activeLesson.sectionIndex}-${activeLesson.lessonIndex}`;
                    ...
                    try {
                      ...
                      const result = await completeLesson(userId, id, lessonId, totalLessons);
                      
                      // Update local state
                      setCompletedLessons(new Set([...completedLessons, lessonId]));
                      setCourseProgress(result.progress.progress);
                      
                      // Show success message with rewards
                      if (result.courseCompleted) {
                        toast({
                          title: "Course Completed! 🎉",
                          description: `You earned ${result.pointsAwarded} points and ${result.xpAwarded} XP!`,
                        });
                      } else {
                        toast({
